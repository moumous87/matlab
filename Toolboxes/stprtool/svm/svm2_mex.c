...
double reg_const1, reg_const2;    /* regularization constant */
...
void *get_col( long a, long b ) 
{
  ...
  if( vector_y[a] == 1) 
     col_ptr[a] = col_ptr[a] + reg_const1;
  else
     col_ptr[a] = col_ptr[a] + reg_const2;
...
void mexFunction( int nlhs, mxArray *plhs[],int nrhs, const mxArray*prhs[] )
{
  ...
  C = mxGetPr(prhs[4]);   /* regularization constants */
  ...
  if( C[0]!=0 ) reg_const1 = 1/C[0]; else reg_const1 = 0;
  if( C[1]!=0 ) reg_const2 = 1/C[1]; else reg_const2 = 0;
  ...
  for(i = 0; i < num_data; i++ ) {
    if(vector_y[i] == 1)
       diagK[i] = 2*kernel(i,i) + reg_const1;
    else
       diagK[i] = 2*kernel(i,i) + reg_const2;
  }
  ...
  for(i = 0; i < num_data; i++ )
  {
     if(vector_y[i] == 1) 
     {
       ...
       if( alpha[i]/(2*C[0]) >= 1 ) trnerr++;
     }
     else
     {
       alpha[i] = -alpha[i]*2/nconst;
       if( alpha[i]/(2*C[1]) <= -1 ) trnerr++;
     }
  }
