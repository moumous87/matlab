...
subplot(2,2,2)
plot(datesp(innd+1:size(port1,1)),[port1(innd+1:end) portn1(innd+1:end) bench(innd+1:end)])
legend('Gross','Net','Benchmark','location','southeast')
grid on
dateaxis('x',12)
title('Absolute Portfolio')
ylim([0,6])
xlim([datesp(innd+1) datesp(size(port1,1))])
...
subplot(2,2,4)
plot(datesp(innd+1:size(port1,1)),[port2(innd+1:end) portn2(innd+1:end) bench(innd+1:end)])
legend('Gross','Net','Benchmark','location','southeast')
grid on
dateaxis('x',12)
title('Relative Portfolio')
...
subplot(2,2,2)
plot(dr(np1+1:end),turn(np1+1:end,1:2))
grid on
dateaxis('x',12)
title('Turnover')
legend('Absolute','Relative')
ylim([0,1.5])
xlim([dr(np1+1) dr(end)])
...
subplot(2,2,4)
plot(dr(np1+1:end),cost(np1+1:end,1:2).*10000)
grid on
dateaxis('x',12)
title('Transaction Costs')
legend('Absolute','Relative')
